...
import {
  AssessmentInitState,
  useAssessmentState,
} from "src/features/assessment/state";
import {
  AssessmentListResponseType,
  ICreateAssessmentPayload,
  createAssessmentAction,
} from "src/features/assessment/action";
...
export default function CreateAssessment() {
  ...
  useEffect(() => {
    setAssessmentState((state) => ({
      ...state,
      assessmentCategories: assessmentCategories.assessmentCategories.map(
        (category) => ({
          id: category.id,
          name: category.name,
        })
      ),
      questionCategories: assessmentCategories.questionCategories.map(
        (category) => ({
          id: category.id,
          name: category.name,
        })
...
//     optionIndex: number
//   ) {
//     questionsModel.questions[questionCategoryIndex].model.selectedOptionIndex =
//       optionIndex;
//     questionsModel.questions[questionCategoryIndex].model.selected = true;
//     questionsModel.questions[questionCategoryIndex].model.value =
//       questionsModel.questions[questionCategoryIndex].model.options[
//         optionIndex
//       ];
//     questionsModel.questions[questionCategoryIndex].category =
//       questionsModel.questions[questionCategoryIndex].model.value?.value ?? "";
...
//   }
  ...
  function validateForm() {
    if (!assessmentTitle?.value?.length) return false;
  ...
  const [questionCategoryVisibility, setQuestionCategoryVisibility] =
    useState(false);
  function openQuestionCategoryModal() {
    setQuestionCategoryVisibility(!questionCategoryVisibility);
  }

  const [assessmentCategoryVisibility, setAssessmentCategoryVisibility] =
    useState(false);
  ...
  return (
    <div className="w-6/12">
        ...
        >
          <InputField
            ...
            onInput={(value: string) =>
              setInput(value, assessmentTitle, setAssessmentTitle)
            }
          />
      ...
      {assessmentCategoryVisibility ? (
        <AddAssessmentCategoryModal closeModal={openAssessmentCategoryModal} />
      ) : null}

      {questionCategoryVisibility ? (
        <AddQuestionCategoryModal closeModal={openQuestionCategoryModal} />
      ) : null}
    </div>
  );
}
