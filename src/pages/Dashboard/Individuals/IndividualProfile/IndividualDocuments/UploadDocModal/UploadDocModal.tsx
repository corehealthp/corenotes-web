...
export default function UploadDocModal({
  ...
  function uploadDocument() {
    if (validateForm()) {
      ...
      JSONToFormData(payload)
          ...
          uploadIndividualDocumentAction(params.individualId!, payloadFormData)
            .then((response) => {
              ...
              createGlobalFeedback(
                "success",
                "Individual document saved successfully"
              );
            })
            .catch(() =>
              createGlobalFeedback(
                "error",
                "There was an error uploading individual document"
              )
            )
            .finally(() =>
        ...
        .catch(() =>
          createGlobalFeedback("error", "There was an error parsing payload")
        )
        .finally(() => {
          setTimeout(() => {
            closeModal();
  ...
  return (
    <ModalContainer
      close={() => (staffState.status === "LOADING" ? null : closeModal())}
    >
      <div className={styles.upload_doc_container}>
        ...
        <div className={styles.form_section}>
          <InputField
            ...
            onInput={(inputValue: string) =>
              setInput(inputValue, docTitleModel, setDocTitleModel)
            }
          />
          ...
          <div className={styles.row}>
            <DropDownField
              ...
              onSelect={(selectedOptionIndex: number) =>
                selectOption(selectedOptionIndex, docTypeModel, setDocTypeModel)
              }
            />
            ...
            <InputField
              ...
              onInput={(inputValue: string) =>
                setInput(inputValue, docDateModel, setDocDateModel)
              }
            />
          </div>
        ...
        <div className={styles.action_buttons}>
          <FadedBackgroundButton
            ...
            width="20%"
            action={() =>
              staffState.status === "LOADING" ? null : closeModal()
            }
          />
